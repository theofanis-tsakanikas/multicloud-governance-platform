...
resource "aws_security_group_rule" "public_orch_ingress" {
  ...
  type              = "ingress"
  description       = "PUBLIC MODE ONLY: 5432 from the CI runners own IP, so Terraform can create the schemas"
  from_port         = 5432
  to_port           = 5432
  protocol          = "tcp"
...
resource "aws_security_group_rule" "rds_ingress_from_ecs" {
  ...
  type                     = "ingress"
  description              = "PRIVATE MODE: 5432 from the pgbouncer gateways security group. The only door into the database"
  from_port                = 5432
  to_port                  = 5432
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "ecs_egress" {
  for_each          = local.private_mode
  type              = "egress"
  description       = "PRIVATE MODE: the gateways outbound to RDS Proxy and the VPC endpoints. There is no IGW on this VPC, so 0.0.0.0/0 here reaches nothing but the VPC itself"
  from_port         = 0
  to_port           = 0
  protocol          = "-1"
  cidr_blocks       = ["0.0.0.0/0"]
