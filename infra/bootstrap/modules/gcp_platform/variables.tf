...
variable "gcp_spn_client_secret"   { type = string; sensitive = true }
variable "provider_key"            { type = string; sensitive = true }
...
variable "delta_sharing_token_lifetime" { type = number; default = 0 }
variable "gcp_delta_sharing_name"  { type = string }
variable "workspace_name"          { type = string }
variable "workspace_pricing_tier"  { type = string; default = "ENTERPRISE" }
