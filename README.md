...
| **[Kept honest by CI](#kept-honest-by-ci)** | OIDC, no long-lived keys, secrets at plan time, Checkov/tfsec, SBOM, 137 tests on every push |
...
> **Left: no rows returned.** The gold tables carry zero PII. **Right: the identities still exist**, in
> the source system, queried in place through federation. They never entered the lakehouse.
...
is SQL over governed tables you can read and rerun, not a black box producing prose. Note the result:
**Azure — no PII datasets.** It did not invent one to fill the gap.
