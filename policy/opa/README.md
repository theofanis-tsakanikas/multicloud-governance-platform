...
[`governance.rego`](governance.rego) is an **independent re-implementation** of the
three gating rules from [`scripts/policy_analyzer.py`](../../scripts/policy_analyzer.py)
(`PUBLIC_PRINCIPAL`, `PII_BROAD_READ`, `SENSITIVE_ALL_PRIVILEGES`) in
...
# The unsafe fixture → 3 denials, one per rule (proves the rules actually fire)
...
same three rules in Python and asserts the Rego policy and the analyzer would
